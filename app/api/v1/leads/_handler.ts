...
export async function moveLeadHandler(
  ...
  const { data: stage, error: stageErr } = await supabase
    .from("crm_stages")
    .select("id, pipeline_id, organization_id, name, afirma_fato, is_lost")
  ...
  //     se move.
  const vereditoDaAfirmacao = podeEntrarNaEtapa(stage as { afirma_fato?: boolean | null }, ctx.actor);
  if (!vereditoDaAfirmacao.permitido) {
    throw new ApiError(
      409,
      "maquina_nao_afirma_fato",
      undefined,
