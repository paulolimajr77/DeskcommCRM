...
export function resolveDestinoDoAgente(
  ...
  const vereditoDaAfirmacao = podeEntrarNaEtapa(alvo, { type: "ai_agent" });
  if (!vereditoDaAfirmacao.permitido) return { move: false, motivo: "etapa_afirma_fato", passo };
...
export async function sincronizaEstagioDoAgente(
  ...
  const { data: stageRows, error: erroStages } = await admin
    ...
    .select("id, name, agent_stage_hint, is_archived, afirma_fato, is_lost")
