...
-- "Erro inesperado" depois de ~30 segundos. O registro do servidor dizia
-- `upstream request timeout`, tres vezes, 10 segundos entre elas.
--
-- O banco NAO era o problema: chamado exatamente como o usuario (mesma sessao,
-- mesmos argumentos, em transacao desfeita), `fn_meet_action` respondeu em 19
-- MILISSEGUNDOS, com a recusa certa (`meet_conversation_stale`).
--
-- A causa, amostrando `pg_locks` DURANTE o clique: varias conexoes do PostgREST
-- disputando a mesma trava por cliente, umas com `granted = t` e outras com
-- `granted = f`. A trava e `pg_advisory_xact_lock` — espera sem teto. O pedido
-- entrava na fila, o porteiro desistia aos 10s, e cada repeticao automatica do
-- cliente punha mais um na fila.
--
-- E a mesma familia do defeito que apagava as regras de isolamento: disputa de
-- trava com o sistema em uso. La o sintoma era tela vazia; aqui, "Erro
-- inesperado".
