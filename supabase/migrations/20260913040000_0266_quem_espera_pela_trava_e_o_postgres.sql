...
-- espera sem teto com um laco de `pg_try_advisory_xact_lock` + `pg_sleep(0.25)`.
-- O laco FUNCIONA — e o mecanismo e ruim:
--
--   • ele PESQUISA em vez de esperar: acorda 4x por segundo para perguntar;
--   • mantem uma conexao do pool do PostgREST acordada enquanto isso;
--   • e so percebe a trava livre ate 250ms DEPOIS de ela ter saido.
--
-- MEDIDO na mesma noite: o banco chegou a 298% de CPU com tres conexoes do
-- PostgREST repetindo `fn_meet_action` sem parar. Reiniciar o PostgREST derrubou
-- para 5,7%. NAO ESTA PROVADO que o laco causou aquilo — o que esta medido e que
-- ele AMPLIFICA qualquer enxurrada, porque troca espera barata por giro caro.
--
-- MEDIDO tambem, e e o que permite o conserto: `lock_timeout` vale para trava de
-- aviso. Sessao segurando `pg_advisory_xact_lock(999111)`, segunda sessao com
-- `lock_timeout='1s'` morreu em "canceling statement due to lock timeout".
-- Entao o Postgres espera por nos — dormindo de verdade, sem CPU, e acordando no
