...
interface CadeiaContagem {
  ...
  /**
   * O filtro de SITUAÇÃO, que encolhe o que bloqueia: só compromisso ainda
   * marcado barra a exclusão — cancelado e concluído acompanham a ficha pela
   * cascade da 0247. Sem `in` aqui o handler estoura `is not a function` e a
   * prova acusa o lugar errado.
   */
  in: (coluna: string, valores: readonly unknown[]) => CadeiaContagem;
...
describe("deleteContactHandler", () => {
    ...
    expect(contagens).toEqual([
      {
        ...
        // O filtro de SITUAÇÃO entra aqui de propósito: só compromisso ainda
        // marcado barra. Cancelado e concluído acompanham a ficha pela cascade
        // da 0247 — sem isso, um contato com uma consulta cancelada em março
        // ficava impossível de excluir, e não havia ação nenhuma pela tela.
        filtros: [
          ["contact_id", CONTATO],
          ["organization_id", ORG],
  ...
  it("contato com compromisso na agenda: 409 e o histórico fica intacto (issue #752)", async () => {
    ...
    await expect(
      ...
      status: 409,
      code: "state_conflict",
      message: expect.stringMatching(/desmarque/i) as unknown as string,
    });
    ...
    expect(ultimaAuditoria()).toMatchObject({
      ...
      organizationId: ORG,
      metadata: {
        motivo: "vinculo_restrict",
        vinculos: ["1 compromisso(s) marcado(s) na agenda"],
        apagados: [],
      },
    });
  });
